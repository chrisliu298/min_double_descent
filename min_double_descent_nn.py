...
def main(x_tr, y_tr, x_te, y_te, P_max, P_step, num_trials):
    ...
    for _ in trange(num_trials):
        ...
        for p in range(P_max, 0, -P_step):
            ...
            z_tr_, y_tr_ = (
                torch.from_numpy(z_tr).float(),
                torch.from_numpy(y_tr).float(),
            )
            W1 = torch.nn.Linear(p, y_tr.shape[1], bias=False)
            torch.nn.init.normal_(W1.weight, mean=0, std=1 / p**0.5)
            optimizer = torch.optim.SGD(W1.parameters(), lr=1e-1, momentum=0.95)
            for _ in range(1000):
                logits = W1(z_tr_)
                loss = torch.nn.functional.mse_loss(logits, y_tr_)
                optimizer.zero_grad(set_to_none=True)
                loss.backward()
                optimizer.step()
            W1 = W1.weight.detach().T.numpy()
